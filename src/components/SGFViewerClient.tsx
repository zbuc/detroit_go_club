...
export default function SGFViewerClient({
  ...
  useEffect(() => {
    let currentBesogoInstance: BesogoInstance | null = null
    ...
    const loadBesoGo = async () => {
      ...
      try {
        ...
        currentBesogoInstance = besogoRef.current
      } catch (error) {
    ...
    return () => {
      // Use captured instance to avoid stale closure warning
      if (currentBesogoInstance) {
        currentBesogoInstance.destroy?.()
      }
    }
